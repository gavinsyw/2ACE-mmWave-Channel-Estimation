...
for i in range(cb_num):
    ...
    brd_name_out = './codebook_brd/multires_16ant_tx/wil6210_tx_cb' + str(i+1) + '.brd' 
    os.system('cp ' + brd_name_in + ' ' + brd_name_out)

    cb_cur = ['00000000000000000000000000000000',
              '00000000000000000000000000000000']
    cb_cur_actual = ['00000000000000000000000000000000',
                    '00000000000000000000000000000000']
    ...
    for sec_idx in range(0, sector_per_cb+2):
    ...
    for sec_idx in range(0, sector_per_cb+2):
        ...
        set_beam(brd_name_out, 0, 1, cur_sec, mag, cb_cur_actual[sec_idx], amp)

    cb_all.extend(cb_cur)
    cb_all_actual.extend(cb_cur_actual)

with open('./codebook_brd/multires_16ant_tx/multires_16_tx.txt', 'w') as f:
     for sector in cb_all:
         f.write("%s\n" % sector)

with open('./codebook_brd/multires_16ant_tx/multires_16_tx_actual.txt', 'w') as f:
