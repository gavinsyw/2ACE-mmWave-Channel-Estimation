function [rowlist,Acriteria] = MyBayesAopt(fxcand,nrows,U,varargin)
...
[emsg,maxiter,xinit,C,K_sub] = statgetargs(okargs,defaults,varargin{:});
A = C'*C;
error(emsg);
...
Minv = zeros(size(X'*X));
for i = 1:U
    Minv = Minv + inv(X'*X+squeeze(K_sub(i,:,:)));
end
MinvF     = Minv*fxcand';
AMinv     = A*Minv;
AMinvF    = AMinv*fxcand';
