function [X, Y, quality] = inferLowRank(A, B, tx, rx, lambda, r, tol_rel, tol_abs, maxiter)
  if nargin < 5, lambda = 0; end
  if nargin < 6, r = 20; end
  if nargin < 7, tol_rel = 1e-4; end
  if nargin < 8, tol_abs = 1e-8; end
  ...
  %----------------------------------------------------------
  [m,n] = size(A);
  train_idx = randsample(m, floor(m*0.95));
  test_idx = setdiff(1:m, train_idx);
  A_train = A(train_idx,:);
  B_train = B(train_idx);
  A_test = A(test_idx,:);
  B_test = B(test_idx);
  [X, Y] = inferLowRankImpl(A_train, B_train, tx, rx, lambda, r, tol_rel, tol_abs, maxiter);
  ...
  if quality > 0.6
    X0 = X;
    Y0 = Y;
    [X, Y] = InferADMM(A, B, X0, true, tx, rx, lambda, tol_rel, tol_abs, maxiter, []);
...
function [X, Y, converged] = inferLowRankImpl(A, B, tx, rx, lambda, r, tol_rel, tol_abs, maxiter)
% 
%   Find X, Y, Z
%
...
%
  ...
  %----------------------------------------------------------
  scale_by_row = true;
  [X, Y] = InferADMM(A, B, X, scale_by_row, tx, rx, lambda, tol_rel, tol_abs, maxiter, U, D);
  ...
  %----------------------------------------------------------
  scale_by_row = false;
  [X, Y, converged] = InferADMM(A, B, X, scale_by_row, tx, rx, lambda, tol_rel, tol_abs, maxiter, U, D);
end
...
%                X=Z
%
function [X,Y,converged] = InferADMM(A, B, X0, scale_by_row, tx, rx, lambda, tol_rel, tol_abs, maxiter, U, D)
...
%
function [Z] = ArgMinZ(X, N, mu, tx, rx)
  Z = X+N/mu;
  ...
  %
  r0 = ceil(sqrt(min([rx tx]))*0.5);
  r1 = ceil(sqrt(min([rx tx]))*0.7);
  r2 = ceil(sqrt(min([rx tx])));
  r3 = ceil(sqrt(min([rx tx]))*2.0);
