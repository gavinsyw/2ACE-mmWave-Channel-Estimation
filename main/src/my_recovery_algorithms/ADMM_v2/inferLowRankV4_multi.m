...
function [X, Y, quality] = inferLowRankV4_multi(A, B, tx, rx, lambda, r, mu0, rho, cc_frac, tol_rel, tol_abs, maxiter)
  ...
  if quality > 0.6
    ...
    [X, Y] = InferADMM(A, B, X_max, true, use_rank_one, tx, rx, lambda, mu0, rho, tol_rel, tol_abs, maxiter, []);    
  end
