function [H_amp,H_angle] = channel_recovery_ADMM_v2_simulation_multiresolution(tx_ant_num, rx_ant_num, cb_amp, cb_angle, rss_final, seed_id)
    ...
    Method.ADMMLowRankV4 = 1;     % Low-rank ADMM
    ...
    seeds = [58659179, 42737934, 36326041, 89830260, 90710947, 96474890, 33424536, 67991541, 42149446, 38961924, 54659060, 32629256, 33087755, 27433950, 9404442, 20146383, 84040563, 75325961, 47726929, 13999319, 5597853, 74801351, 37024073, 75534492, 99245881, 19650488, 5314224, 98859252, 60803022, 76056701, 14112116, 64027813, 73073690, 6288587, 42217659, 45632040, 7495955, 31960297, 92863244, 93081516];
    rng(seeds(seed_id));
    ...
    for i = 1: length(M)
        ...
        if M_cur <= thresh(1)
            M_idx = randperm(res_separation(1), M_cur);
        elseif (M_cur > thresh(1)) && (M_cur <= thresh(2))
            M_idx = randperm(res_separation(2), M_cur) + res_separation(1);
        else
            M_idx = randperm(res_separation(3), M_cur) + res_separation(1)+res_separation(2);
        end
