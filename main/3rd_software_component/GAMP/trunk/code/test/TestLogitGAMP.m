...
% binary classification problem using the LogitEstimOut class.  In this
...
% Last change: 08/30/12
% Change summary: 
%       - Created from TestProbitGAMP v0.2 (08/30/12; JAZ)
% Version 0.2
%
...
lagrangian = 1e-1;   % Lagrangian penalty term for SoftThreshEstimIn and
                    % EllpEstimIn (lambda)
elastic_lam = [1e0, 1e-1];   % [ell-1, ell-2] penalty terms for elastic net
...
% Logit regression function parameters
% The output channel is specified as p(y = 1 | z) = (1 + exp(-alpha*z))^-1,
% where alpha is a scalar parameter that controls the steepness of the
% sigmoid function
alpha = 1e0;        % Sigmoid scale parameter
map_gamp = true;   % Run MMSE (false) or MAP (true) version of GAMP?
...
RunTFOCS = true;                % Include TFOCS in comparison?
...
%         x(active_features) = exprnd(lagrangian,K_true,1) - exprnd(lagrangian,K_true,1);
    case false
        x = zeros(N,1);
...
% Build the EstimOut object for a logit regression channel
EstimOut = LogitEstimOut(y_train, alpha, [], [], map_gamp);
...
%% Compute error metrics for turboGAMP
...
if strcmpi(sigprior, 'laplace') && map_gamp
    PMonesY = 2*(y_train - 1/2);    % {0,1} -> {-1,1}
    % Compute convergence vector
    cvec = (alpha*PMonesY)./(1 + exp(alpha*PMonesY.*(A_train*x_hat)));
    cvec = A_train' * cvec;
...
elseif strcmpi(sigprior, 'elastic') && map_gamp
    PMonesY = 2*(y_train - 1/2);    % {0,1} -> {-1,1}
    % Compute convergence vector
    deriv = (alpha*PMonesY)./(1 + exp(alpha*PMonesY.*(A_train*x_hat)));
    cvec = A_train' * deriv - 2*elastic_lam(2)*x_hat;
