% PRODUCE_PLOTS     Produce plots for the MMV GM2 phase plane test
%

%
% Coded by: Justin Ziniel, The Ohio State Univ.
% E-mail: zinielj@ece.osu.edu
% Last change: 02/20/12
% Change summary: 
...
for i = 1:Q
    for j = 1:Q
        TNMSE_sks_extract = TNMSE_sks(i,j,1:n);
        TNMSE_sks_extract = TNMSE_sks_extract(TNMSE_sks_extract < 1e1);
        if isempty(TNMSE_sks_extract), TNMSE_sks_extract = NaN; end
        mean_TNMSE_sks_dB(i,j) = 10*log10(median(TNMSE_sks_extract, 3));
        mean_TNMSE_turbo_dB(i,j) = 10*log10(median(TNMSE_turbo(i,j,1:n), 3));
        mean_TNMSE_naive_dB(i,j) = 10*log10(median(TNMSE_naive(i,j,1:n), 3));
...
% % Plot the mean time-averaged NMSE for all completed trials on a dB scale
% Cnum = 10;      % # of contour lines
% subplot(131); [C1, H1] = contour(delta, fliplr(beta), flipud(mean_TNMSE_sks_dB), Cnum);
% clabel(C1, H1)
% subplot(132); [C2, H2] = contour(delta, fliplr(beta), flipud(mean_TNMSE_turbo_dB), Cnum);
% clabel(C2, H2)
% subplot(133); [C3, H3] = contour(delta, fliplr(beta), flipud(mean_TNMSE_naive_dB), Cnum);
% clabel(C3, H3)

% Cnum = 10;      % # of contour lines
% subplot(121); [C2, H2] = contour(delta, fliplr(beta), flipud(mean_TNMSE_turbo_dB), [-35:2:-17]);
% clabel(C2, H2, 'fontsize', 18)
% subplot(122); [C3, H3] = contour(delta, fliplr(beta), flipud(mean_TNMSE_naive_dB), [-35:4:-1]);
...
subplot(121); [C2, H2] = contour(delta, fliplr(beta), ...
    flipud(mean_TNMSE_turbo_dB - mean_TNMSE_sks_dB), 15);
clh1 = clabel(C2, H2); hold on
subplot(122); [C3, H3] = contour(delta, fliplr(beta), ...
    flipud(mean_TNMSE_naive_dB - mean_TNMSE_sks_dB), 15);
