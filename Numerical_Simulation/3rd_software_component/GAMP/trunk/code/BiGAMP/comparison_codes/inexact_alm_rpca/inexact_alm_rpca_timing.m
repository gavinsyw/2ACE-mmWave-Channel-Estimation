function [A_hat,E_hat,iter,time_data,estHist] = inexact_alm_rpca_timing(D, lambda, tol, maxIter)
...
%Preallocate storage
estHist.Zhat = zeros(m*n,maxIter);
...
while ~converged       
    ...
    %Save result
    estHist.Zhat(:,iter) = reshape(A_hat,[],1); 
    
    
    %stopCriterion = norm(Z, 'fro') / d_norm;
    stopCriterion = norm(estHist.Zhat(:,iter) - Zold) /...
        norm(estHist.Zhat(:,iter));
    if stopCriterion < tol
        converged = true;
    end
    Zold = estHist.Zhat(:,iter);
...
estHist.Zhat = estHist.Zhat(:,1:iter);
