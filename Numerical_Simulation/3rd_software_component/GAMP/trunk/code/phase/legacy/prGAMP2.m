% prGAMP2 Phase-Retrieval Generalized Approximate Message Passing 2.0
%
%   prGAMP2 uses Rangan's Generalized AMP algorithm to perform phase 
...
% SYNTAX: 
%
%   [xhat,out,estHist] = prGAMP2(abs_y, A, opt_pr)
%
% DEMO MODE: 
%
...
%   opt_pr.SNRdB : SNR (in dB), used to guide re-tries and stopping (default=60)
%   opt_pr.maxTry : max number of tries from random initializations (default=10)
...
%   opt_pr.nitEM : max number of EM iterations to tune SNRdB (default=20...less is faster)
%   opt_pr.SNRdBinit : SNR (in dB) to start EM iterations at (default=10)
...
function [xhat,out,estHist_] = prGAMP2(abs_y,A,opt_pr,opt_gamp_user)
%nargin=0; nargout=1;
 ...
 SNR = 10^(SNRdB/10);
 wvar = (mean(abs_y.^2)/(SNR+1));  % corresponding noise variance
 ...
 if exist('opt_pr')&&isfield(opt_pr,'SNRdBinit'), 
   ...
   SNRdBinit = opt_pr.SNRdBinit;
 else 
   SNRdBinit = 10;   % default is 10 dB 
 end;
 SNRinit = 10^(SNRdBinit/10);
 wvar_init = (mean(abs_y.^2)/(SNRinit+1));  % initial noise variance
...
%  init_gain = min(100,1/sparseRat^2)   % default gain on random initializations
 ...
 if exist('opt_pr')&&isfield(opt_pr,'adaptStep'), 
   ...
   adaptStep = 1;       % default is to have adaptive stepsize on
 end;
 ...
 opt_gamp_init.pvarStep = 1;            % seems important [dflt:1]
 ...
 for t=1:maxTry,
   ...
   support_size = nan;
   nit_ = nan(1,nitEM+1);
   if nitEM==0, last_em=true; else last_em=false; end   % last EM iteration
   
   % loop over EM iterations
   for em=1:nitEM+1
       
     % setup for current EM iteration
     outputEst = ncCAwgnEstimOut(abs_y,wvar_hat(em)*ones(nz,1)); 
     ...
     if computeHist
       ...
       %figure(1); clf; gampShowHist(estHist,optFin,x,z)
     ...
     if print_on,
       if em==1, fprintf(1,'\nAttempt number %d:\n',t); end;
       if nargin==0,
         %nmseGAMPdB(em) = 20*log10(norm(x*sign(x'*xhat)-xhat)/norm(x));
     ...
     %isnonzero = sum(abs(estFin.rhat).^2 > 1e-3*sparseRat*(xmean0.^2+xvar0));
     ...
     if (norm(xhat_old-estFin.xhat)/norm(estFin.xhat) < tolEM )||(em==nitEM)  % mainly use this 
       if last_em,
         break;
       else
         last_em = true;
     ...
     if last_em,
       switch EMtype
         case 1
           wvar_hat(em+1) = mean(estFin.zvar+2*(abs_y-abs(estFin.zhat)).^2);%
         case 2
           wvar_hat(em+1) = mean(A.multSq(estFin.xvar)+2*(abs_y-abs(A.mult(estFin.xhat))).^2);
         case 3
           zhat = estFin.zhat; zvar = estFin.zvar;
           xi = -(abs(zhat).^2)./zvar;
           zmagbar = 0.5*sqrt(pi*zvar).*((1-xi).*besseli(0,-0.5*xi,1)-xi.*besseli(1,-0.5*xi,1));
           %zmagbar = abs(zhat);
           zmag2bar = zvar + abs(zhat).^2;
           kappa = 2*abs_y.*abs(zhat)/wvar_hat(em);
           scale = 2;
           %wvar_hat(em+1) = scale*mean(abs( abs_y.^2 + zmag2bar -2*abs_y.*zmagbar.*sqrt(1+0.25./(kappa.^2)) ));% 
           wvar_hat(em+1) = scale*mean(abs( abs_y.^2 + zmag2bar -2*abs_y.*zmagbar ));% 
         case 4
           zhat = A*estFin.xhat; zvar = A.multSq(estFin.xvar);
           xi = -(abs(zhat).^2)./zvar;
           zmagbar = 0.5*sqrt(pi*zvar).*((1-xi).*besseli(0,-0.5*xi,1)-xi.*besseli(1,-0.5*xi,1));
           %zmagbar = abs(zhat);
           zmag2bar = zvar + abs(zhat).^2;
           ...
           wvar_hat(em+1) = scale*mean(abs( abs_y.^2 + zmag2bar -2*abs_y.*zmagbar ));% 
         case 11
           wvar_hat(em+1) = mean(2*(abs_y-abs(estFin.zhat)).^2); 
         case 12
       ...
       switch EMtype
         case 1
           wvar_hat(em+1) = mean(2*(abs_y-abs(estFin.zhat)).^2);% no zvar term 
         case 2
           wvar_hat(em+1) = mean(2*(abs_y-abs(A*estFin.xhat)).^2);% no zvar term 
         case 3
           zhat = estFin.zhat; zvar = estFin.zvar;
           xi = -(abs(zhat).^2)./zvar;
           zmagbar = 0.5*sqrt(pi*zvar).*((1-xi).*besseli(0,-0.5*xi,1)-xi.*besseli(1,-0.5*xi,1));
           %zmagbar = abs(zhat);
           zmag2bar = abs(zhat).^2; % remove zvar term to increase EM convergence rate
           kappa = 2*abs_y.*abs(zhat)/wvar_hat(em);
           scale = 2;
           %wvar_hat(em+1) = scale*mean(abs( abs_y.^2 + zmag2bar -2*abs_y.*zmagbar.*sqrt(1+0.25./(kappa.^2)) ));% 
           wvar_hat(em+1) = scale*mean(abs( abs_y.^2 + zmag2bar -2*abs_y.*zmagbar ));%
 ...
 if nargin==0
   ...
   wvar_over_true_dB = 10*log10(wvar_best/wvar)
 end
