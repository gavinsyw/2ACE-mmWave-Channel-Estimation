% prGAMP4 Phase-Retrieval Generalized Approximate Message Passing version 4.0
%
%   prGAMP4 uses Rangan's Generalized AMP algorithm to perform phase 
...
%   opt_pr.tuneOn : automatically tune signal sparsity-rate & variance? (default=1)
%   opt_pr.sparseRat : initial signal sparsity rate in (0,1] (default=1)
%   opt_pr.SNRdBinit : SNR (in dB) to initialize noise variance (default=10)
%   opt_pr.SNRdB : SNR (in dB) used for terminating re-starts (default=10)
...
%   opt_pr.xvar0 : length-N vector of initial non-zero-signal variances
...
function [xhat,out,estHist_] = prGAMP4(abs_y,A,opt_pr,opt_gamp_user)
%nargin=0; nargout=1;
 ...
 if nargin==0,
  ...
  opt_pr.tuneOn = true;                 % tune, starting with default sparsity rate
  ...
  z = A*x;
  wvar_true = 10^(-SNRdB_true/10)*mean(abs(z).^2);
  w = sqrt(wvar_true/2)*(randn(nz,2)*[1;1i]);
  y = z + w;
  abs_y = abs(y);
 ...
 if exist('opt_pr')&&isfield(opt_pr,'SNRdB'), 
   ...
   SNRdB = 10;          % default is 10 dB (was 60 dB)
 end;
 SNR = 10^(SNRdB/10);
 wvar = (mean(abs_y.^2)/(SNR+1));  % used only for PO-LMMSE & PO-GAMP

 % initial SNR in dB used for EM noise-learning iterations
 ...
 % maximum number of EM noise-learning iterations 
 ...
 else % set so that nz~=2*k*log2(nx/k)
   k=1; while (nz>2*k*log2(nx/k))&&(k<nx), k=k+1; end
   sparseRat = k/nx; 
 end;
 ...
 % Choose type of EM noise learning
 ...
 % Choose type of sparsity/signal-variance tuning
 if exist('opt_pr')&&isfield(opt_pr,'tuneOn'), 
   tuneOn = opt_pr.tuneOn;
 else
   tuneOn = true; % default is on
 end
 if xnonneg && tuneOn, 
   tuneOn=false; 
   warning('setting tuneOn=false since tuning not yet implemented for NNGMEstimIn!'); 
 end 
 tuneSpar = tuneOn; % tune signal sparsity?
 tuneVar = tuneOn; % tune variance of non-zero signal coefficients?
 if tuneOn
   sparseRat = min(sparseRat,0.9); % if tuning, keep initial sparsity<1
 end
 ...
 if xreal
   if xnonneg
     ...
     inputEst0 = AwgnEstimIn(xmean0, xvar0, false,...
                        'mean0Tune',false, 'autoTune',tuneVar);
   end
 else
   inputEst0 = CAwgnEstimIn(xmean0, xvar0, false,... 
                        'mean0Tune',false, 'autoTune',tuneVar);
 end;
 if sparseRat < 1, 
   inputEst = SparseScaEstim(inputEst0,sparseRat,0,...
                        'autoTune',tuneSpar);
 else
   inputEst = inputEst0;
 end;
 ...
 for t=1:maxTry,
   ...
   if tuneOn % restore tuning initialization 
     if any(strcmp('p1', properties(inputEst))) % SparseScaEstim is being used
       inputEst.p1 = sparseRat; 
       inputEst.estim1.mean0 = xmean0; 
       inputEst.estim1.var0 = xvar0; 
     else % SparseScaEstim is not being used
       inputEst.mean0 = xmean0; 
       inputEst.var0 = xvar0; 
     end;
   end;
   
   % prepare for EM noise-learning iterations
   ...
   support_size = nan;
   nit_ = nan(1,nitEM+1);
   xmean0sq_hat = nan(1,nitEM+1);
   xvar0_hat = nan(1,nitEM+1);
   sparseRat_hat = nan(1,nitEM+1);
   ...
   for em=1:nitEM+1
     ...
     if em==1
       ...
       if any(strcmp('disableTune', properties(inputEst)))
         inputEst.disableTune = true; % temporarily turn off input tuning 
       end;
     else %em>1
       ...
       if any(strcmp('disableTune', properties(inputEst)))
         inputEst.disableTune = false; % restore input tuning (if applicable)
       end;
     end
     ...
     % extract current tuning values
     if tuneOn
       if any(strcmp('p1', properties(inputEst))) % SparseScaEstim is being used
         sparseRat_hat(em)= inputEst.p1;
         xmean0sq_hat(em) = mean(abs(inputEst.estim1.mean0).^2);
         xvar0_hat(em) = mean(inputEst.estim1.var0);
       else % SparseScaEstim is not being used
         sparseRat_hat(em)= 1;
         xmean0sq_hat(em) = mean(abs(inputEst.mean0).^2);
         xvar0_hat(em) = mean(inputEst.var0);
       end
     end
   ...
   xmean0sq_hat = xmean0sq_hat(1:em);
   xvar0_hat = xvar0_hat(1:em);
   sparseRat_hat = sparseRat_hat(1:em);
 ...
 out.xvar0 = xvar0;
 out.wvar = wvar_best;
 out.sparseRat_hat = sparseRat_hat;
 out.xvar0_hat = xvar0_hat;
 ...
 if nargin==0
   ...
   wvarHat_over_wvarTrue_dB = 10*log10(wvar_best/wvar_true)
   % report accuracy of sparsity estimate
   if tuneOn, 
     sparseRatHat_over_sparseRatTrue = sparseRat_hat/sparseRat_true, 
     xvar0hat_over_xvar0true = xvar0_hat/mean(xvar0_true), 
   end;
 end
