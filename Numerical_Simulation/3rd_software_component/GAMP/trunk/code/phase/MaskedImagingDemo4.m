% MaskedImagingDemo:  Demo of phase-retrieval GAMP on an image, using multiple binary masks
 ...
 switch DEMO
   ...
   case 2 % compressive N=256^2-pixel image recovery from M=N/2 magnitudes using post-randomized and pre-masked DFTs 
    ...
    SNRdB_true = 30; % SNR in dB [30]
    plot_gamp = 0;  % plot residual trajectory for debugging? (slows things down)
   case 3 % like case 2 but with N=512^2-pixel images
    image_type = 2;
    num_masks = 2;  % number of masks [2]
    num_band = 10;  % number of non-zero diagonals in banded randomization matrix [10]
    nZ_over_ndft = sqrt(0.5/num_masks); % Fourier sampling ratio, per dimension  [0.5]
    ...
    SNRdB_true = 30; % SNR in dB [30]
    plot_gamp = 0;  % plot residual trajectory for debugging? (slows things down)
   case 4 % like case 2 but with N=1024^2-pixel images
    image_type = 3;
    num_masks = 2;  % number of masks [2]
    num_band = 10;  % number of non-zero diagonals in banded randomization matrix [10]
    nZ_over_ndft = sqrt(0.5/num_masks); % Fourier sampling ratio, per dimension  [0.5]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 % set PR-GAMP options
 clear opt_pr;
 opt_pr.SNRdB = SNRdB_true;     % set stopping tolerances based on true
 opt_pr.SNRdBinit = SNRdB_true; % initialize noise variance from true SNR
opt_pr.nitEM = 0;              % EM iterations
 opt_pr.xreal = 1;              % exploit real-valuedness of image 
 opt_pr.xnonneg = 1;            % exploit non-negativity of image
 opt_pr.sparseRat = sparseRat;  % exploit sparsity of image (if sparseRat<1)
%opt_pr.xmean0 = xmean0;        % exploit mean of non-zero signal elements (optional)
%opt_pr.xvar0 = xvar0;          % exploit variance of non-zero signal elements (optional)
 opt_pr.verbose = 1;            % print per-attempt residual [dflt=0]
 opt_pr.plot = plot_gamp;       % figure to plot trajectory (or "0" for no plot)
 opt_pr.maxTry = 10;            % maximum number of re-starts [dflt=10]
 opt_pr.init_gain = 1;          % gain on random initialization [dflt=1]

 % set exceptions to GAMP options in prGAMP
 clear opt_gamp;
 opt_gamp.uniformVariance = 1;  % since faster and more robust here [dflt=1]
 opt_gamp.adaptStep = 0;        % no adaptive stepsize
 opt_gamp.stepMax = 0.35;       % somewhat aggressive max stepsize [dflt=0.35]
 opt_gamp.step = opt_gamp.stepMax; % start at max stepsize [dflt=stepMax]

 % run PR-GAMP
 tstartGAMP = tic;
 [xhat,out_pr] = prGAMP4(y_abs,A,opt_pr,opt_gamp);
out_pr
 ...
 figure(1); clf;
 cmap = colormap('gray');
 scale = max(abs(x(:)))/size(cmap,1);
 subplot(221)
   image(reshape(x/scale,nX1,nX2))
   axis('equal');axis('tight'); 
   title('true')
 subplot(222)
   image(reshape(xhat*sign(xhat'*x)/scale,nX1,nX2))
   colormap('gray')
   axis('equal');axis('tight'); 
...
%T=100; mse=nan(1,T); time=nan(1,T); for t=1:T, MaskedImagingDemo4; mse(t)=mseGAMPdB; time(t)=timeGAMP; end; mse_worst = max(mse), median(time)
